...
### How does the server identify your CLI?

**It doesn't, in the strict sense.** DCR is an **anonymous** registration mechanism (the endpoint runs with `auth='none'`). You declare a `client_name` — purely cosmetic, it's what the user sees on the consent screen — and the server returns a fresh `client_id` it just generated. **That `client_id` IS the identity** of this registration. The CLI must persist it (e.g. `~/.config/<your-cli>/client.json`) and reuse it for all subsequent flows.

Two distribution modes follow from this:

| Mode | Identity | Persistence |
|---|---|---|
| **Per-install DCR** (this tutorial's path) | Each install runs Step 0 once → its own `client_id`. | The CLI persists `client_id` locally. Losing the file forces a re-registration. |
| **Pre-registered static client** (used by first-party CLIs) | A fixed `client_id` ships with the binary (e.g. `mpy-cli`, `mgx-cli` in [`data/oauth_clients.xml`](../data/oauth_clients.xml)). | All installs of the same CLI share one server-side registration. Step 0 is skipped. |

**Trust model.** The server cannot authenticate your CLI binary (it's a public client with no secret). Trust comes from the **user at the consent screen** (Step 3) deciding whether to grant access to a client identified by `<client_name>`. Pick a `client_name` honest enough that users can recognise it.
